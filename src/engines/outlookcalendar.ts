...
async function getCalendarView(q: string/*start: any, end: any, timeZone: any*/) {
    if (!auth) {
    ...
    const events = await auth
        ...
        .select('id,lastModifiedDateTime,start,end,webLink,subject,bodyPreview,body,organizer')
        // Order by start time
    ...
    return events.value.map((e: Event) => {
        ...
        return {
            ...
            title: `${e.subject} on ${date.toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' })
                }`,
            url: e.webLink
        }
    });
